...
public:
    ...
    std::unordered_map<std::string, OrderBookMarketDataUpdate> lastUpdateData;
    ...
    double calculateHistoricalVaR(
        ...
        const std::unordered_map<std::string, std::vector<OrderBookMarketDataUpdate>> &historicalData,
        ...
        int horizonDays) override
    {
        ...
        for (const auto &[symbol, updates] : historicalData)
        {
            ...
            std::sort(sortedUpdates.begin(), sortedUpdates.end(),
                      [](const OrderBookMarketDataUpdate &a, const OrderBookMarketDataUpdate &b)
                      {
    ...
    int checkCircuitBreakers(
        const std::vector<OrderBookMarketDataUpdate> &marketData,
        const std::unordered_map<std::string, Position> &positions,
        const std::unordered_map<std::string, double> &thresholds) override
    {
        ...
        std::unordered_map<std::string, std::vector<OrderBookMarketDataUpdate>> symbolData;
    ...
    // temp trading halts that kick in during periods of sverse price decline to ensure that info
    //  if disseminated, and liquidity managed
    bool checkCircuitBreaker(const MarketDataUpdate &update)
    {

        auto it = lastUpdateData.find(update.symbol);
        if (it != lastUpdateData.end())
        {
            const MarketDataUpdate &prevUpdate = it->second;

            double priceChange = (update.lastPrice - prevUpdate.lastPrice) / prevUpdate.lastPrice;
            OrderBookOrder order;
            double price_ = order.price;
            if (std::abs(priceChange) > price_)
            {
                std::cout << "Circuit Breaker Triggered: Price change exceeded threshold for "
                          << update.symbol << ": " << (priceChange * 100) << "%" << std::endl;
                return true;
            }

            double volumeSpike = update.volume / prevUpdate.volume;
            if (volumeSpike > price_)
            {
                std::cout << "Circuit Breaker Triggered: Volume spike exceeded threshold for "
                          << update.symbol << ": " << volumeSpike << "x normal volume" << std::endl;
                return true;
            }
        }

        lastUpdateData[update.symbol] = update;

        return false;
    }
    // New Position=Current Position+Order Quantity
    bool validateOrder(const Order &order, const Position &position)
    {
        //                                     price = limit price
        if (order.quantity + position.quantity > order.price)
        {
            return false;
        }
        return true;
    ...
    Position getPosition(const std::string &symbol) const
    {
        ...
        {
            // return default posiiton is not found
            return Position();
        }
    }
    ...
    double calculateVolatility(const std::vector<MarketDataUpdate> &marketData)
    {
        ...
        for (int i = 1; i < n; ++i)
        {
            double prevPrice = marketData[i - 1].lastPrice;
            double currentPrice = marketData[i].lastPrice;
    ...
    double calculateRisk(const OrderBookOrder &order, const Position &position)
    {

        double basePositionRisk = calculatePositionRisk(position);

        double newQuantity = position.quantity;
        if (order.side == OrderBookOrderSide::BUY)
        {
        ...
        double positionValue = std::abs(newQuantity * position.marketPrice);

        double positionRisk = positionValue / maxPositionLimit_;

        if (positionRisk > basePositionRisk && basePositionRisk > 0.5)
        {
            positionRisk = 1.2;
        }
    ...
    double calculatePositionRisk(const Position &position)
    {
        ...
        if (position.quantity == 0 || position.marketPrice <= 0)
        {
            return 0.0;
        }

        double positionValue = std::abs(position.quantity) * position.marketPrice;

        double positionRisk = positionValue / maxPositionLimit_;

        if (position.unrealizedPnl < 0)
        {
            double lossPercentage = std::abs(position.unrealizedPnl) / positionValue;
            positionRisk *= (1.0 + lossPercentage);
        }
    ...
    std::unordered_map<std::string, double> HistoricalVaRCalculator::getRiskReport(
        const std::unordered_map<std::string, Position> &positions)
    {
        ...
        double totalValue = 0.0, totalLong = 0.0, totalShort = 0.0;
        for (const auto &[symbol, pos] : positions)
        {
            totalValue += pos.marketValue;
            if (pos.quantity > 0)
                totalLong += pos.marketValue;
            else
                totalShort += std::abs(pos.marketValue);
        }

        report["TOTAL_VALUE"] = totalValue;
        report["TOTAL_LONG"] = totalLong;
        report["TOTAL_SHORT"] = totalShort;

        return report;
    }

    void RiskCalculator::updatePosition(const Position &position)
    {
        positions_[position.symbol] = position;
    }

    void HistoricalVaRCalculator::updatePosition(const Position &position)
    {
        positions_[position.symbol] = position;
    }
};
