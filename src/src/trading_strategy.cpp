...
/*
TradingStrategy::TradingStrategy(const std::string &name)
{
    std::string name_(name);
}


Order TradingStrategy::generateOrder(const MarketDataUpdate &update)
{
    Order order;
    order.symbol = update.symbol;

    // Times 2 cause we risky bois
    if (update.lastPrice * 2 > update.closePrice)
    {
        order.price = update.askPrice;
        order.side = OrderSide::BUY;
    }
    else
    {
        order.price = update.bidPrice;
        order.side = OrderSide::SELL;
    }

    // Will finish later, get back later
    order.quantity = update.bidSize;
    order.type = OrderType::MARKET;
    order.status = OrderStatus::NEW;
    order.timestamp = update.timestamp;
    order.clientId = "Ryan_is_awesome_123";
    order.strategyId = "We_making_money_4_life";
    return order;
}

void TradingStrategy::updateMarketConditions(const MarketDataUpdate &update)
{
    double midPrice = (update.bidPrice + update.askPrice) / 2.0;
    double spread = update.askPrice - update.bidPrice;

    // New MarketState instance
    MarketState ms;
    ms.midPrice = midPrice;
    ms.spread = spread;
    ms.volume = update.volume;
    ms.lastUpdateTime = update.timestamp;

    // Compute the volatility
    auto it = marketStates_.find(update.symbol);
    if (it != marketStates_.end())
    {
        double previousMidPrice = it->second.midPrice;
        ms.volatility = (previousMidPrice > 0.0)
                            ? std::abs(midPrice - previousMidPrice) / previousMidPrice
                            : 0.0;
    }
    else
    {
        // No previous data, so initialize volatility to 0.
        ms.volatility = 0.0;
    }

    marketStates_[update.symbol] = ms;
}
double calculateKellyFraction(double expectedReturn, double riskFreeRate, double volatility)
{
    ...
    double kellyFraction = (expectedReturn - riskFreeRate) / variance;
    return kellyFraction;
}
double TradingStrategy::calculateOptimalSize(const std::string &symbol) const
{
    // Kelly Criterion
    double p = 0.90;
    double l = 3.0;
    double a = 0.5;
    double expectedReturn = (p / l);
    double riskFreeRate = ((1 - p) / a);
    return calculateKellyFraction(expectedReturn, riskFreeRate, 0.5);
}
...
public:
    MeanReversionStrategy(const std::string &name) : TradingStrategy(name)
    {
        ...
        std::string deviationThreshold_;
    ...
    std::vector<Order> onMarketDataUpdate(const MarketDataUpdate &marketData) override
    {
        ...
        if (marketData.timestamp % (1000000000LL * 60) < 1000000)
...
bool MeanReversionStrategy::initialize(const std::unordered_map<std::string, std::string> &params)
{
    try
    {
        auto it = params.find("lookbackPeriod");
        ...
        if (it != params.end())
        {
            ...
            while ((pos = symbolsStr.find(',')) != std::string::npos)
            {
                watchlist_.insert(symbolsStr.substr(0, pos));
                symbolsStr.erase(0, pos + 1);
            }
            if (!symbolsStr.empty())
            {
        ...
        {
            priceHistory_[symbol] = std::vector<double>();
            priceHistory_[symbol].reserve(lookbackPeriod_ * 2);
            for (const auto &otherSymbol : watchlist_)
            {
                if (symbol != otherSymbol)
                {
...
std::vector<Order> MeanReversionStrategy::onMarketDataUpdate(const MarketDataUpdate &marketData)
{
    if (!isActive_ || watchlist_.find(marketData.symbol) == watchlist_.end())
        return {};
    ...
    if (zScore > deviationThreshold_)
    {
        double volatility = stdDev / movingAvg;
        int orderSize = calculateOptimalOrderSize(marketData.lastPrice, volatility, riskLimit_);
        if (orderSize > 0)
        {
            Order sellOrder;
            ...
            sellOrder.type = OrderType::LIMIT;
            sellOrder.quantity = orderSize;
            sellOrder.price = marketData.bidPrice * 0.999;
            sellOrder.timestamp = marketData.timestamp;
            sellOrder.status = OrderStatus::NEW;
            sellOrder.strategyId = getName();
            orders.push_back(sellOrder);
    ...
    else if (zScore < -deviationThreshold_)
    {
        ...
        if (orderSize > 0)
        {
            ...
            buyOrder.type = OrderType::LIMIT;
            buyOrder.quantity = orderSize;
            buyOrder.price = marketData.askPrice * 1.001;
            buyOrder.timestamp = marketData.timestamp;
            buyOrder.status = OrderStatus::NEW;
            buyOrder.strategyId = getName();
            orders.push_back(buyOrder);
...
int MeanReversionStrategy::calculateOptimalOrderSize(double price, double volatility, double maxRisk)
{
    int maxSize = static_cast<int>(positionLimit_ / price);
    std::vector<std::pair<int, double>> dp(maxSize + 1, {0, 0.0});
    ...
    for (const auto &[size, retRisk] : dp)
    {
        if (price * size * volatility <= maxRisk)
        {
            return size;
        }
    }
...
std::vector<std::vector<double>> MeanReversionStrategy::analyzeMarketRelationships(const std::vector<MarketDataUpdate> &marketData)
{
    std::unordered_map<std::string, std::vector<double>> symbolPrices;
    for (const auto &update : marketData)
    {
    ...
    std::vector<std::vector<double>> result(watchlist_.size(), std::vector<double>(watchlist_.size(), 0.0));
    int i = 0;
    for (const auto &symbol : watchlist_)
    {
        int j = 0;
        for (const auto &otherSymbol : watchlist_)
        {
            result[i][j] = (symbol == otherSymbol) ? 1.0 : correlationMatrix_[symbol][otherSymbol];
            j++;
        }
        i++;
    }
...
std::vector<MarketDataUpdate> MeanReversionStrategy::processMarketData(const std::vector<MarketDataUpdate> &marketData)
{
    std::vector<MarketDataUpdate> filteredData;
    std::copy_if(marketData.begin(), marketData.end(), std::back_inserter(filteredData),
                 [this](const MarketDataUpdate &update)
                 {
                     return watchlist_.find(update.symbol) != watchlist_.end();
                 });
    ...
    if (!filteredData.empty())
    {
        ...
        filteredData.erase(std::remove_if(filteredData.begin(), filteredData.end(),
                                          [latestTimestamp](const MarketDataUpdate &update)
                                          {
                                              return latestTimestamp - update.timestamp > 5000000000;
                                          }),
                           filteredData.end());
    }
...
double MeanReversionStrategy::calculateMovingAverage(const std::string &symbol, int period)
{
    const auto &prices = priceHistory_[symbol];
    if (prices.size() < static_cast<size_t>(period))
        return 0.0;
...
double MeanReversionStrategy::calculateStandardDeviation(const std::string &symbol, int period, double mean)
{
    const auto &prices = priceHistory_[symbol];
    if (prices.size() < static_cast<size_t>(period))
        return 0.0;
...
void MeanReversionStrategy::calculateCorrelationMatrix(const std::unordered_map<std::string, std::vector<double>> &symbolPrices)
{
    for (const auto &[symbol1, prices1] : symbolPrices)
    {
        for (const auto &[symbol2, prices2] : symbolPrices)
        {
            if (symbol1 != symbol2)
            {
                ...
                if (minLength > 1)
                {
                    correlationMatrix_[symbol1][symbol2] = calculateCorrelation(
                        std::vector<double>(prices1.end() - minLength, prices1.end()),
                        std::vector<double>(prices2.end() - minLength, prices2.end()));
                }
            }
...
double MeanReversionStrategy::calculateCorrelation(const std::vector<double> &x, const std::vector<double> &y)
{
    double sum_x = std::accumulate(x.begin(), x.end(), 0.0);
    double sum_y = std::accumulate(y.begin(), y.end(), 0.0);
...
void MeanReversionStrategy::findRelatedSecurities()
{
    for (const auto &symbol : watchlist_)
    {
        std::priority_queue<std::pair<double, std::string>> correlatedSymbols;
...
MeanReversionStrategy::MeanReversionStrategy(const std::string &name)
    : TradingStrategy(name)
{
    // Optional: set strategy-specific parameters
}
Order MeanReversionStrategy::generateOrder(const MarketDataUpdate &update)
{
    Order order;
    order.symbol = update.symbol;
    order.type = OrderType::MARKET;
    order.side = OrderSide::BUY;
    order.price = update.lastPrice;
    order.quantity = 10; // Hardcoded for now
    order.timestamp = update.timestamp;
    order.orderId = static_cast<uint64_t>(rand()) % 1000000;

    return order;
}

double MeanReversionStrategy::calculateSpread(const std::string &symbol) const
{
    // Dummy spread
    return 0.02;
}

double MeanReversionStrategy::calculateOptimalSize(const std::string &symbol) const
{
    // Dummy optimal size
    return 100;
}
