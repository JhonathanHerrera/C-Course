...
public:
    ...
    // Order validation
    bool validateOrder(const OrderBookOrder &order, const Position &position);

    // Risk metrics
    virtual double calculateRisk(const std::string &symbol) const = 0;
...
public:
    ...
    double calculateRisk(const std::string &symbol) const override;
};