...
std::pair<double, double> OrderBookProcessor::calculateMarketImpact(
    const std::string &symbol, double quantity, OrderSide side)
{

    // Calculate the immediate price impact
    double immediateImpact = calculatePriceImpact(symbol, quantity, side);

    // First value: price impact on the bid side
    // Second value: price impact on the ask side
    if (side == OrderSide::BUY)
    {
        return {0.0, immediateImpact}; // Impact on ask side
    }
    else
    {
        return {immediateImpact, 0.0}; // Impact on bid side
    }
}
